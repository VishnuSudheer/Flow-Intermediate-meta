...
// Vstoken contract: Implements fungible token on Flow blockchain
pub contract Vstoken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @Vstoken.Vault
        ...
        // Destroy the vault
        destroy() {
            Vstoken.totalSupply = Vstoken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            Vstoken.totalSupply = Vstoken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&Vstoken.Minter>(/public/Minter, target: /storage/MinterStorage)
